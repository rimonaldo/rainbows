...
import { Color, hsl } from '../../services/color'
...
import { HarmonyTitle } from '../../services/harmony'
...
export class Palette implements PaletteType {
   ...
   getSecondaryHue(primaryHue: number, validHueAngles: number[], secondaryHueDirection: number) {
      const randomHueIndex = Math.floor(Math.random() * validHueAngles.length)
      const hueAngle = validHueAngles[randomHueIndex]
      return secondaryHueDirection > 0 ? primaryHue + hueAngle : primaryHue - hueAngle
   }

   setSecondary = () => {
      const maxOffset = 10
      const validHueAngles = [180,60,45]
      const primaryHue = this.primary.shade[500].hsl.h
      const offset = Math.floor(Math.random() * maxOffset)
      const secondaryHueDirection = Math.floor(Math.random() * 2) === 0 ? -1 : 1
      const targetHue = this.getSecondaryHue(primaryHue, validHueAngles, secondaryHueDirection)
      let accentColor = new PaletteColor({ color: new Color({ hex: '#000' }), role: 'secondary' })
      let nonce = 0
      while (nonce < 500) {
         const AAhex = getRandomAAColor(this.primary.shade[500].hex)
         accentColor = new PaletteColor({ color: new Color({ hex: AAhex }), role: 'secondary' })

         if (accentColor.color.hsl.h > targetHue - offset && accentColor.color.hsl.h < targetHue + offset) {
            return accentColor
         }
      ...
      return accentColor
      // Fallback
      // let newColor = new Color({ hsl: { h, s: this.primary.color.hsl.s, l: this.primary.color.hsl.l } })
